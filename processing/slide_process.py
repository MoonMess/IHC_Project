...
from processing.tile_process import update_size, filter_enough_zone, get_tile, process_tile, save_patch
...
def get_slide(slide_folder):
    ...
    #slide_filenames = list(filter(lambda filename: '8037' in filename, slide_filenames))
    #slide_filenames = list(filter(lambda filename: 'RP' in filename, slide_filenames))

    #slide_filenames = list(filter(lambda filename: '250001010_H171793_RP' in filename, slide_filenames))
...
def process_slide(slide_filename, slide_folder):
    ...
    ihc_rgb2 = hed2rgb(np.stack((ihc_hed[:, :, 0]*3, ihc_hed[:, :, 1], ihc_hed[:, :, 2]*3), axis=-1))
    ...
    for nbr, (i, j) in enumerate(tqdm(selected_i_and_j)):
        ...
        #save_patch(tile, nbr, slide_filename)
